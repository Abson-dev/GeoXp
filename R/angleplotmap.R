...
####################################################
 ...
 pointfunc <- function() 
 {
  ...
  while (!quit) 
  {
   ...
   obs <<- selectstat(var1 = theta, var2 = absvar, obs = obs,Xpoly = loc[1], Ypoly = loc[2], method = "AnglePoint",long = long, lat = lat)
   diag(obs)<<-FALSE
   graphique(var1 = theta, var2 = absvar, obs = obs,num = 3, graph = "Angleplot", labvar = labvar,symbol = symbol, quantiles = quantiles,alpha1 = alpha)
   carte(long = long, lat = lat, obs = obs,buble=buble,criteria=criteria,nointer=nointer,cbuble=z,carte=carte,nocart=nocart, lablong = lablong,lablat = lablat, label = label, symbol = symbol,method = "Angleplot",axis=axis,legmap=legmap,legends=legends) 
 }
}
...
if (length(polyX)>0)
{
        ...
        for (i in 1:length(long)) 
       {
        obs[, i] <<- inout(cbind(theta[, i], absvar[, i]),cbind(polyX, polyY), bound = TRUE)
        diag(obs)<<-FALSE
       }
...
cartfunc <- function()
{
  if(!nocart)
   { if (length(carte) != 0)
       {nocart<<-TRUE
        carte(long=long, lat=lat,criteria=criteria,buble=buble,cbuble=z,nointer=nointer,obs=obs, lablong=lablong, lablat=lablat,method="Angleplot", label=label, symbol=symbol,carte=carte,nocart=nocart,axis=axis,legmap=legmap,legends=legends) 
       }
     else
       {
        tkmessageBox(message="Spatial Contours have not been given",icon="warning",type="ok")
        carte(long=long, lat=lat,criteria=criteria,nointer=nointer,buble=buble,cbuble=z,obs=obs, lablong=lablong,method="Angleplot", lablat=lablat, label=label, symbol=symbol,axis=axis,legmap=legmap,legends=legends) 
       }
  }
 ...
 {
   nocart<<-FALSE;
   carte(long=long, lat=lat,buble=buble,criteria=criteria,nointer=nointer,cbuble=z,obs=obs,method="Angleplot", lablong=lablong, lablat=lablat, label=label, symbol=symbol,carte=carte,nocart=nocart,axis=axis,legmap=legmap,legends=legends) 
