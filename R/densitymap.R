...
{graph<<-"Densityplot1"  
    ...
    while(!quit)
    {
        ...
        if (length(var[obs]) >= 2)
        {
            graphique(var1=var, obs=obs, alpha1=alpha1,  num=3, graph="Densityplot1", labvar=labvar, symbol=symbol,kernel=kernel);
...
#    quit <- FALSE;
    ...
    # graphiques
    graphique(var1=var, obs=obs, alpha1=alpha1, num=3, graph="Densityplot2", Xpoly=polyX[1], Ypoly=polyX[2], labvar=labvar, symbol=symbol,kernel=kernel);
 if ((graphChoice != "") && (varChoice1 != "") && (length(dev.list()) > 2))
           {if(graphChoice=="Barplot")
             {
                labmod<-levels(as.factor(listvar[,which(listnomvar == varChoice1)]))
...
if(graph=="Densityplot1") 
    {graphique(var1=var, obs=obs, alpha1=alpha1,  num=3, graph=graph, labvar=labvar, symbol=symbol,kernel=kernel);}
else
    {graphique(var1=var, obs=obs, alpha1=alpha1,  num=3, graph="Densityplot2", Xpoly=interv[1], Ypoly=interv[2], labvar=labvar, symbol=symbol,kernel=kernel);}
  }
####################################################
