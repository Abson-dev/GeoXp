...
ginifunc<-function()
{
 ...
 while(!quit)
  ...
  #  while (length(ptX)<1)
    ...
    # graphiques
    graphique(var1=var, obs=obs, num=3, graph="Lorentz", Xpoly=ptX, labvar=labvar, symbol=symbol, F=F, G=G);
...
tkfocus(tt1)              
    ...
    {
        tkmessageBox(message="Listvar and listnomvar must have been given",icon="warning",type="ok");
    }
  }
...
SGfunc<-function() 
{
 ...
 # graphiques
    ...
    graphique(var1=var, obs=obs, num=3, graph="Lorentz", labvar=labvar, symbol=symbol, F=F, G=G);
...
obs<<-(var<=angle)
  graphique(var1=var, obs=obs, num=3, graph="VLorentz", Xpoly=angle, labvar=labvar, symbol=symbol, F=F, G=G);
  if ((graphChoice != "") && (varChoice1 != "") && (length(dev.list()) > 2))
           {if(graphChoice=="Barplot")
             {
                labmod<-levels(as.factor(listvar[,which(listnomvar == varChoice1)]))
               graphique(var1=listvar[,which(listnomvar == varChoice1)], var2=listvar[,which(listnomvar == varChoice2)], obs=obs, num=4, graph=graphChoice, symbol=1, labvar=c(varChoice1,varChoice2),labmod=labmod,col=color);
               carte(long=long, lat=lat,buble=buble,cbuble=z,criteria=criteria,nointer=nointer,obs=obs,  label=label, symbol=symbol,carte=carte,nocart=nocart,method="Cluster",classe=listvar[,which(listnomvar == varChoice1)],col=color,legmap=legmap,legends=legends,labmod=labmod)
             }
           else
            {
            graphique(var1=listvar[,which(listnomvar == varChoice1)], var2=listvar[,which(listnomvar == varChoice2)], obs=obs, num=4, graph=graphChoice, symbol=1, labvar=c(varChoice1,varChoice2));
            carte(long=long, lat=lat,obs=obs,buble=buble,cbuble=z,criteria=criteria,nointer=nointer,  label=label, symbol=symbol,carte=carte,nocart=nocart,legmap=legmap,legends=legends)
            } 
           }
       else
         {carte(long=long, lat=lat,obs=obs,buble=buble,cbuble=z,criteria=criteria,nointer=nointer,  label=label, symbol=symbol,carte=carte,nocart=nocart,legmap=legmap,legends=legends)
...
tkfocus(tt1)
    ...
    carte(long=long, lat=lat, obs=obs,  label=label, symbol=symbol, lablong="", lablat="",axis=axis)
graphique(var1=var, obs=obs, num=3, graph="Lorentz", labvar=labvar, symbol=symbol, F=F, G=G);
