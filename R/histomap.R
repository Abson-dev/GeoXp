    ...
    while(!quit)
     {
      ...
      graphique(var1=var, obs=obs, num=3, graph="Histogram", nbcol=nbcol, labvar=labvar, symbol=symbol);
    ...
    if ((length(listvar) != 0) && (length(listnomvar) != 0))
    {
      ...
      if (((graphChoice == "Histogram")&&(!is.numeric(listvar[,which(listnomvar == varChoice1)])))||((graphChoice == "Scatterplot")&&((!is.numeric(listvar[,which(listnomvar == varChoice1)]))||(!is.numeric(listvar[,which(listnomvar == varChoice2)]))))) 
        ...
        tkmessageBox(message="Variables choosed are not in a good format",icon="warning",type="ok");
         ...
         { if((graphChoice == "Barplot")&&(length(levels(factor(listvar[,which(listnomvar == varChoice1)])))>10))
           {
            tkmessageBox(message="Variable choosed has too many factors",icon="warning",type="ok");
           }    
           else
           { 
...
carte(long=long, lat=lat,criteria=criteria,nointer=nointer,buble=buble,cbuble=z,obs=obs,  label=label, symbol=symbol,carte=carte,legmap=legmap,legends=legends,axis=axis,lablong=lablong,lablat=lablat)
graphique(var1=var, obs=obs, num=3, graph="Histogram", nbcol=nbcol, labvar=labvar, symbol=symbol);
