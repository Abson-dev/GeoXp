...
if(length(labvar)==0)
 {labvar<-names(data.frame(var1))}

if(labmod[1]=="")
 {labmod<-levels(as.factor(var2))}
...
boxfunc<-function()
{
    ...
    while(!quit)
    {
        ...
        obs<<-selectstat(var1=var1, var2=var2,obs=obs,Xpoly=loc[1], Ypoly=loc[2],method="Polyboxplot");

        # graphiques
        graphique(var1=var1, var2=var2, obs=obs, num=3, graph="Polyboxplot", labvar=labvar, symbol=symbol,col=color,labmod=labmod);
        carte(long=long, lat=lat,buble=buble,cbuble=z,criteria=criteria,nointer=nointer,obs=obs, lablong=lablong, lablat=lablat, label=label, symbol=symbol,carte=carte,nocart=nocart,col=color,method="Cluster",classe=var2,legmap=legmap,legends=legends,labmod=labmod,axis=axis)
  ...
  if(!nocart)
   { if (length(carte) != 0)
     ...
     else
       {
        ...
        carte(long=long, lat=lat,criteria=criteria,nointer=nointer,buble=buble,cbuble=z,obs=obs, lablong=lablong, lablat=lablat, label=label, symbol=symbol,col=color,method="Cluster",classe=var2,legmap=legmap,legends=legends,labmod=labmod,axis=axis)
       }
  }
